...
public static class PowerShellFormatter
{
    ...
    public static FormatterResult Format(string source, FormatterOptions? options = null)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(options), "IndentSize must be between 0 and 32.");
        }
    ...
    private static string FormatBraces(string source, FormatterOptions options)
    {
        ...
        var hashtableBraces = ast
            .FindAll(node => node is HashtableAst, searchNestedScriptBlocks: true)
            .Cast<HashtableAst>()
        ...
        for (var index = 0; index < tokens.Length; index++)
        {
            ...
            if (token.Kind == TokenKind.LCurly && !hashtableBraces.Contains(token.Extent.StartOffset))
            {
                ...
                if (next is not null && next.Kind != TokenKind.RCurly && next.Kind != TokenKind.NewLine)
                {
                    ReplaceWhitespaceBetween(source, token, next, newLine, edits);
                }
            }
            else if (token.Kind == TokenKind.RCurly && !hashtableBraces.Contains(token.Extent.StartOffset))
            {
                var previous = PreviousSignificant(tokens, index);
                var next = NextSignificant(tokens, index);
                if (previous is not null && previous.Kind is not (TokenKind.LCurly or TokenKind.NewLine))
                {
                    ReplaceWhitespaceBetween(source, previous, token, newLine, edits);
                }

                if (next is not null && next.Kind != TokenKind.NewLine && IsCuddledKeyword(next.Kind))
                {
    ...
    private static string FormatWhitespace(string source, FormatterOptions options)
    {
        ...
        for (var index = 0; index < tokens.Length; index++)
        {
            ...
            else if (options.SpaceAroundPipe && token.Kind is TokenKind.Pipe or TokenKind.AndAnd or TokenKind.OrOr)
            {
                var previous = PreviousSignificant(tokens, index);
                var next = NextSignificant(tokens, index);
                if (previous is not null && previous.Kind != TokenKind.NewLine)
    ...
    private static string FormatIndentation(string source, FormatterOptions options)
    {
        ...
        {
            for (var line = token.Extent.StartLineNumber + 1; line <= token.Extent.EndLineNumber; line++)
            {
        ...
        var tokensByLine = tokens
            ...
            .ToDictionary(group => group.Key, group => group.OrderBy(token => token.Extent.StartOffset).ToArray());
        var depth = 0;

        for (var lineIndex = 0; lineIndex < lines.Length; lineIndex++)
        {
            var lineNumber = lineIndex + 1;
            if (!tokensByLine.TryGetValue(lineNumber, out var lineTokens) || protectedLines.Contains(lineNumber))
            {
                continue;
            }

            var first = lineTokens[0];
            var lineDepth = first.Kind == TokenKind.RCurly ? Math.Max(0, depth - 1) : depth;
    ...
    private static Token? NextSignificant(Token[] tokens, int index)
    {
        ...
        {
            if (tokens[cursor].Kind != TokenKind.Comment && tokens[cursor].Kind != TokenKind.EndOfInput)
            {
                return tokens[cursor];
            }
        }
    ...
    private static string MakeIndent(int depth, FormatterOptions options) => options.UseTabs
        ? new string('\t', depth)
    ...
    private static IReadOnlyList<FormatterParseError> ToErrors(ParseError[] errors) => errors
