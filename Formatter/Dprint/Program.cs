...
{
    // PowerShell's parser registers these CIM type accelerators through reflection.
    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(CimInstance))]
    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(CimClass))]
    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(CimType))]
    [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(CimConverter))]
...
public static class Plugin
{
    ...
    {
        var result = PowerShellFormatter.Format(
            source,
            ParseOptions(configJson, overrideConfigJson)
        );
        return result.Text;
    }
    ...
    public static string GetConfigDiagnostics(string configJson)
    {
        ...
        try
        {
            ...
            if (
                document.RootElement.TryGetProperty("plugin", out var plugin)
    ...
    public static string GetResolvedConfig(string configJson)
    {
        ...
        return $$"""
            {"braceStyle":"{{braceStyle}}","indentSize":{{options.IndentSize}},"useTabs":{{Boolean(
                options.UseTabs
            )}},"correctKeywordCasing":{{Boolean(
                options.CorrectKeywordCasing
    ...
    {
        using var stream =
            typeof(Plugin).Assembly.GetManifestResourceStream("Formatter.Dprint.LICENSE")
            ?? throw new InvalidOperationException(
                "The embedded plugin license could not be found."
            );
    ...
    private static string GetVersion()
    {
        var version =
            ...
            ?? throw new InvalidOperationException(
                "The plugin assembly version could not be read."
            );
    ...
    private static void ApplyPluginOptions(JsonElement plugin, FormatterOptions options)
    {
        ...
        if (
            plugin.TryGetProperty("braceStyle", out var braceStyle)
            && braceStyle.ValueKind == JsonValueKind.String
        )
        {
    ...
    private static string SerializeDiagnostics(
        IEnumerable<(string PropertyName, string Message)> diagnostics
    )
    {
