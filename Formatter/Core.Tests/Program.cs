...
Check("hashtable remains inline", "$x=@{one=1;two=2}", "$x = @{one = 1; two = 2}");
...
Check(
    "multiline parameter indentation",
    "[CmdletBinding()]\nparam (\n[Parameter(Mandatory)]\n[string] $Path\n)",
    "[CmdletBinding()]\nparam (\n    [Parameter(Mandatory)]\n    [string] $Path\n)"
);
...
Console.WriteLine("8 formatter checks passed");
return 0;
